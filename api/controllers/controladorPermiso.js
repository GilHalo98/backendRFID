...
exports.consultaPermiso = async(request, respuesta) => {
    // GET Request.
    ...
    try {
        ...
        if(consulta.descripcionPermiso) {
            datos.descripcionPermiso = {
                [Op.substring]: consulta.descripcionPermiso
            };
        }

        // Consultamos el total de los registros.
        const totalRegistros = await Permisos.count({
            where: datos,
        });

        // Consultamos todos los registros.
        const registros = await Permisos.findAll({
            offset: offset,
            limit: limit,
            where: datos
        });
...
exports.registrarPermiso = async(request, respuesta) => {
    // POST Request.
    const cabecera = request.headers;
    const cuerpo = request.body;
    const parametros = request.params;
...
exports.modificarPermiso = async(request, respuesta) => {
    // PUT Request.
    const cabecera = request.headers;
    const cuerpo = request.body;
    const parametros = request.params;
...
exports.eliminarPermiso = async(request, respuesta) => {
    // DELETE Request.
    const cabecera = request.headers;
    const cuerpo = request.body;
    const parametros = request.params;
    ...
    try {
        ...
        if(!permiso) {
            // Retorna un error.
            return respuesta.status(200).send({
                codigoRespuesta: CODIGOS.PERMISO_NO_ENCONTRADO,
            });
        }
